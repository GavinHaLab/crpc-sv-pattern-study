...
# Only difference is how to prepare "bed.combined" (compare to makeSVmatrixByGenes_with_H3K27ac_enhancer_for_intraSV.R)
# Create artificial partner breakpont depends on orientation (if it's forward, set `End` as length of chromosome, if it's reverse, set `Start` as 0)
# This way we can apply same Findoverlap logic for inter SV data.
...
outImageAnalysis <- paste0("./results/longSV_analysis/",outPrefix,"_",transectType,".RData")
...
for (annotType in annotTypes){
	...
	for (i in samples){
		...
		bed.inter.1 <- bed[, -c("chrom2","start2","end2","orient_2")]
		bed.inter.2 <- bed[, -c("chrom1","start1","end1","orient_1")]
		setnames(bed.inter.1, c("chrom1","start1","end1","orient_1"), c("chr","start","end","orient"))
		setnames(bed.inter.2, c("chrom2","start2","end2","orient_2"), c("chr","start","end","orient"))
		bed.inter.bind <- rbind(bed.inter.1, bed.inter.2)

		#fwd : replace new end for chr length 
		#rev : replace new start for 0
		bed.inter.bind[orient=="fwd", end := chrEnds[chr==Chromosome,V1],by=chr]
		bed.inter.bind[orient=="rev", start := 0]

		#bed.combined <- rbind(bed.intra, bed.inter.bind, fill=TRUE)

		bed.combined <- bed.inter.bind
		bed.combined.gr <- as(bed.combined, "GRanges")

		#run this for each gene (a.k.a flanking) specific region!
		for(r in regions.uniq){
			...
			##any contains evne when entire bed region covers gene regions
...
## output files ##
outFile <- paste0("./results/longSV_analysis/",outPrefix, "_summary_H3K27ac_", transectType, ".bedpe")
...
outFile <- paste0("./results/longSV_analysis/",outPrefix, "_enhUpMatrix_H3K27ac_", transectType, ".txt")
writeMatrixToFile(enhancerUpMat, outFile)

outFile <- paste0("./results/longSV_analysis/",outPrefix, "_enhDownMatrix_H3K27ac_", transectType, ".txt")
writeMatrixToFile(enhancerDownMat, outFile)


outImageMats <- paste0("./results/longSV_analysis/",outPrefix, "_mats_H3K27ac_", transectType, ".RData")
